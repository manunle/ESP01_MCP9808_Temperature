...
struct strConfig {
  ...
  String NUHeartbeatTopic;                // NOT USED up to 32 Byte - EEPROM 222
  long HeartbeatEvery;                  // 4 Byte - EEPROM 254
  long TempSeconds;                      // 4 Byte - EEPROM 260
  int LowTempAlarm;                     // 2 Byte - EEPROM 264
  int HighTempAlarm;                     // 2 Byte - EEPROM 268
  // need to add bool SendTempMessages, int AlarmRepeatSeconds
  // Application Settings here... from EEPROM 270 up to 511 (0 - 511)
  ...
  void WriteConfig(){
    ...
    EEPROMWritelong(260,config.TempSeconds);                      // 4 Byte - EEPROM 296
    EEPROMWriteint(264,config.LowTempAlarm);                     // 2 Byte - EEPROM 300
    EEPROMWriteint(268,config.HighTempAlarm);                     // 2 Byte - EEPROM 302
  ...
  boolean ReadConfig(){
    ...
    if (EEPROM.read(0) == cfgver[0] && EEPROM.read(1) == cfgver[1] && EEPROM.read(2) == cfgver[2] && EEPROM.read(3) == cfgver[3] )
    {
      ...
      config.TempSeconds = EEPROMReadlong(260);                      // 4 Byte - EEPROM 296
      config.LowTempAlarm = EEPROMReadint(264);                     // 2 Byte - EEPROM 300
      config.HighTempAlarm = EEPROMReadint(268);                     // 2 Byte - EEPROM 302
      // Application parameters here ... from EEPROM 192 to 511
...
void printConfig(){
  ...
  Serial.printf("Heartbeat Every %ld seconds",config.HeartbeatEvery);                  // 4 Byte - EEPROM 292
  Serial.printf("Read temp Every %ld seconds",config.TempSeconds);                     // 4 Byte - EEPROM 296
  Serial.printf("Low temp alarm:%d",config.LowTempAlarm);                     // 2 Byte - EEPROM 300
  Serial.printf("High temp alarm:%d",config.HighTempAlarm);                     // 2 Byte - EEPROM 302
    // Application Settings here... from EEPROM 192 up to 511 (0 - 511)
